...
function groupStack(lineHeight, item, group, groupHeight, groupTop, itemIndex, groupMinHeight) {
  ...
  if (item.dimensions.stack && item.dimensions.top === null) {
    ...
    if (groupMinHeight && groupMinHeight > 0 && curHeight < groupMinHeight) {//tony cts
      curHeight = groupMinHeight;
    }
    do {
...
function stackAll(itemsDimensions, groupOrders, lineHeight, stackItems, groupMinHeight) {
  var groupHeights = [];
  var groupTops = [];
  ...
  for (var index in groupedItems) {
    ...
    var _stackGroup = stackGroup(_itemsDimensions, isGroupStacked, lineHeight, groupTop, groupMinHeight),
...
function stackGroup(itemsDimensions, isGroupStacked, lineHeight, groupTop, groupMinHeight) {
  var groupHeight = 0;
  ...
  for (var itemIndex = 0; itemIndex < itemsDimensions.length; itemIndex++) {
    ...
    if (isGroupStacked) {
      r = groupStack(lineHeight, itemsDimensions[itemIndex], itemsDimensions, groupHeight, groupTop, itemIndex, groupMinHeight);
    } else {
 ...
 * @param {number} groupMinHeight
 */


function stackTimelineItems(items, groups, canvasWidth, canvasTimeStart, canvasTimeEnd, keys, lineHeight, itemHeightRatio, stackItems, draggingItem, resizingItem, dragTime, resizingEdge, resizeTime, newGroupOrder, groupMinHeight) {
  ...
  var _stackAll = stackAll(dimensionItems, groupOrders, lineHeight, stackItems, groupMinHeight),
